...
export default class HeatMap {
  ...
  addHeatPoints() {
    ...
    for (let i = 0; i < this.coordinates.length; i++) {
      const heatPoint = new HeatPoint(this.coordinates[i].x, this.coordinates[i].y, this.coordinates[i].sst, this.gameArea);
  ...
  resetTemp() {
    for (let i = 0; i < this.heatPoints.length; i++) {
      this.heatPoints[i].sst = this.coordinates[i].sst;
    }
  }
