...
export default class Hurricane extends GameObject {
  ...
  getHitCity(hitList) {
    for (let i = 0; i < hitList.length; i++) {
      if (hitList[i].x === 526 && !this.activityLog.includes(`Hurricane hit Honolulu, HI`)) {
        this.activityLog.unshift(`Hurricane hit Honolulu, HI`);
      } else if (hitList[i].x === 717 && !this.activityLog.includes(`Hurricane hit Los Angeles, CA`)) {
        this.activityLog.unshift(`Hurricane hit Los Angeles, CA`);
      } else if (hitList[i].x === 710 && !this.activityLog.includes(`Hurricane hit Seattle, WA`)) {
        this.activityLog.unshift(`Hurricane hit Seattle, WA`);
      } else if (hitList[i].x === 192 && !this.activityLog.includes(`Hurricane hit Tokyo, Japan`)) {
        this.activityLog.unshift(`Hurricane hit Tokyo, Japan`);
      } else if (hitList[i].x === 80 && !this.activityLog.includes(`Hurricane hit Hong Kong, China`)) {
        this.activityLog.unshift(`Hurricane hit Hong Kong, China`);
      } else if (hitList[i].x === 105 && !this.activityLog.includes(`Hurricane hit Manila, Philippines`)) {
